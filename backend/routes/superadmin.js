...
const initSuperAdmin = async () => {
  const existing = await SuperAdmin.findOne({ email: 'superadmin@expressiontracker.com' });
  if (!existing) {
    const hashed = await bcrypt.hash('superadmin123', 10);
    await SuperAdmin.create({ email: 'superadmin@expressiontracker.com', password: hashed });
